...
CQ_Analytics.CampaignMgr.prototype.getCampaignBy=function(e,b){if(!this.data||!this.data.campaigns||typeof this.data.campaigns==="string"){return null
...
CQ_Analytics.CampaignMgr.prototype.getExperienceBy=function(g,d){if(!this.data||!this.data.campaigns||typeof this.data.campaigns==="string"){return null
...
if(!CQ_Analytics.DlgMgr){CQ_Analytics.DlgMgr=function(){};
CQ_Analytics.DlgMgr.prototype=new CQ_Analytics.PersistedSessionStore();
CQ_Analytics.DlgMgr.prototype.STOREKEY="DLGSTORE";
CQ_Analytics.DlgMgr.prototype.STORENAME="dlgstore";
CQ_Analytics.DlgMgr.prototype.DEFAULT_EXPERIENCE="DEFAULT";
CQ_Analytics.DlgMgr.prototype.init=function(){this.persistence=new CQ_Analytics.SessionPersistence({container:"",mode:CQ.shared.ClientSidePersistence.MODE_SESSION});
var a;
if(Modernizr.sessionstorage){a=this.persistence.getWindow().sessionStorage.getItem(this.STOREKEY)
}else{a=this.persistence.getWindow()[this.STOREKEY]
}if(a){this.data=this.parse(a)
}else{this.data={}
}this.initialized=true
};
CQ_Analytics.DlgMgr.prototype.setProperty=function(a,b){if(this.data){this.data[a]=b
}if(this.isPersisted(a)&&this.persistence){if(Modernizr.sessionstorage){this.persistence.getWindow().sessionStorage.setItem(this.STOREKEY,this.toString())
}else{this.persistence.getWindow()[this.STOREKEY]=this.toString()
}}};
CQ_Analytics.DlgMgr.prototype.setProperties=function(a){var b=false;
_.forOwn(a,function(d,c){this.data[c]=d;
if(!b&&this.isPersisted(c)){b=true
}},this);
if(b){this.persist()
}};
CQ_Analytics.DlgMgr.prototype.getProperty=function(a){if(this.data){return this.data[a]
}};
CQ_Analytics.DlgMgr.prototype.removeProperty=function(a){if(this.data[a]){delete this.data[a];
if(this.isPersisted(a)){this.persist()
}}};
CQ_Analytics.DlgMgr.prototype.clear=function(){this.data={};
this.initProperty={};
this.nonPersisted=[];
this.persist()
};
CQ_Analytics.PersistedSessionStore.prototype.isPersisted=function(a){this.nonPersisted=this.nonPersisted||[];
return this.nonPersisted[a]!==true
};
CQ_Analytics.DlgMgr.prototype.parse=function(d){var c=function(g){if(!g||typeof(g)!="string"){return g
}var f=g;
f=f.replace(/&#44;/g,",");
f=f.replace(/&#61;/g,"=");
f=f.replace(/&#124;/g,"|");
return f
},b=function(g){var f=null;
if(typeof JSON!=="undefined"&&typeof JSON.parse==="function"){f=JSON.parse(g)
}else{f=$.parseJSON(g)
}return f
};
var a={},e=d.split(",");
_.forEach(e,function(g){var f=g.split("=");
if(f.length==2){a[f[0]]=b(c(f[1]))
}});
return a
};
CQ_Analytics.DlgMgr.prototype.toString=function(){var b="";
if(this.data){var a=function(d){if(!d||typeof(d)!="string"){return d
}var c=d;
c=c.replace(/,/g,"&#44;");
c=c.replace(/=/g,"&#61;");
c=c.replace(/\|/g,"&#124;");
...
_.forOwn(this.data,function(d,c){if(this.isPersisted(c)){if(b){b+=","
}b+=(c+"="+a(JSON.stringify(d)))
}},this)
}return b
};
CQ_Analytics.DlgMgr.prototype.persist=function(){if(Modernizr.sessionstorage){this.persistence.getWindow().sessionStorage.setItem(this.STOREKEY,this.toString())
}else{this.persistence.getWindow()[this.STOREKEY]=this.toString()
}};
CQ_Analytics.DlgMgr=new CQ_Analytics.DlgMgr();
CQ_Analytics.CCM.addListener("configloaded",function(){CQ_Analytics.CCM.register(this)
},CQ_Analytics.DlgMgr)
}if(!CQ_Analytics.CampaignMgr){CQ_Analytics.CampaignMgr=function(){};
CQ_Analytics.CampaignMgr.prototype=new CQ_Analytics.PersistedSessionStore();
